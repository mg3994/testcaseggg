...
> **❗ Import** This is still in development. While the language server is being used by Code OSS, it has not yet been tested with other clients.

The Markdown language server powers Code OSS's built-in markdown support, providing tools for writing and browsing Markdown files. It runs as a separate executable and implements the [language server protocol](https://microsoft.github.io/language-server-protocol/overview).
